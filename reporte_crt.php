...
$titulo_reporte_hoja = "Reporte CRT";
    ...
    if ($id == NULL) { // AL MENOS UNO DE LOS DATOS QUE SOLICITA EL FORMULARIO DE BUSQUEDA
        ...
        if (($estado_habilitacion_usuario + $estado_habilitacion_cliente) > 1) { // SISTEMA HABILITADO ...
            ...
            while ($via_actual < $reporte_cant_vias) {    // ----------------------------------------------------------------      WHILE DE VIAS               --------------------------------------------------------------
                ...
                // $pdf->SetFont($tipo_fuente, '', 10); // ajustar el tamanho de fuente que se va a imprimir

                // $pdf->SetX(20);

                // $pdf->Cell(242, 5, utf8_decode($texto), '', $siguiente_linea, 'R', $rellenar);
                ...
                // $pdf->Cell(18, 5, utf8_decode($texto), '', $siguiente_linea, 'R', $rellenar);
                ...
                if ($resultados_busqueda_estandar = $conexion_bd->query($sent_buscar)) {
                    ...
                    while ($filas_resultado_estandar = $resultados_busqueda_estandar->fetch_assoc()) {    // obtener array asociativo 
                        ...
                        $suma_valor += $filas_resultado_estandar['valor_crt'];


                        $texto = fecha_salida_corta_sin_hora($filas_resultado_estandar['f_doc'], '/');
                        ...
                        $texto = $filas_resultado_estandar['n_pedido'];

                        $pdf->Cell(12, $alto_renglon_celda, utf8_decode($texto), $bordes, $siguiente_linea, $alineacion, $rellenar);



                        $texto = substr($filas_resultado_estandar['rem_nombre'], 0, 15);
                        ...
                        $texto = substr($filas_resultado_estandar['des_nombre'], 0, 15);
                        ...
                        $texto = substr($filas_resultado_estandar['con_nombre'], 0, 15);
                        ...
                        $texto = substr($filas_resultado_estandar['lugar_embarque'], 0, 15);

                        $pdf->Cell(25, $alto_renglon_celda, utf8_decode($texto), $bordes, $siguiente_linea, $alineacion, $rellenar);


                        
                        $texto = substr($filas_resultado_estandar['lugar_entrega'], 0, 10);

                        $pdf->Cell(20, $alto_renglon_celda, utf8_decode($texto), $bordes, $siguiente_linea, $alineacion, $rellenar);



                        $texto = substr($filas_resultado_estandar['descripcion_producto'], 0 ,25);

                        $pdf->Cell(40, $alto_renglon_celda, utf8_decode($texto), $bordes, $siguiente_linea, $alineacion, $rellenar);


                        $alineacion = "L";
                        $texto = $filas_resultado_estandar['u_medida_peso_bruto'];
                        ...
                        $texto = formatMoneda($filas_resultado_estandar['peso_neto'], $id_moneda);

                        $pdf->Cell(15, $alto_renglon_celda, utf8_decode($texto), $bordes, $siguiente_linea, $alineacion, $rellenar);    

                        

                        $texto = formatMoneda($filas_resultado_estandar['peso_bruto'], $id_moneda);
                        ...
                        $texto = formatMoneda($filas_resultado_estandar['valor_crt'], $id_moneda);
                ...
                $bordes = "T";
                        ...
                        $texto = formatMoneda($suma_valor, $id_moneda);
